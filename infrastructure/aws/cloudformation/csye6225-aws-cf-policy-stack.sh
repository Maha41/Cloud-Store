...
then
    ...
    aws cloudformation create-stack --stack-name $stackname --template-body file://csye6225-cf-policy.json --parameters ParameterKey=IamUser,ParameterValue=circleci --capabilities CAPABILITY_NAMED_IAM 
    if [ $? -eq 0 ]
    then
        while true;
        do
