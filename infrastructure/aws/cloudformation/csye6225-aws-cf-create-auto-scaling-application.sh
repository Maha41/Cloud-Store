...
aws cloudformation validate-template --template-body file://csye6225-cf-auto-scaling-application.json --output text
...
then
    ...
    nameExtension4="-csye6225-subnet4"
    subnetName4=$vpcStack$nameExtension4
    Subnet4=$(aws ec2 describe-subnets --filters "Name=tag:Name,Values=$subnetName4"  --query 'Subnets[*].SubnetId' --output text 2>&1)
    echo "SubnetId4: $Subnet4"
    nameExtension5="-csye6225-subnet5"
    subnetName5=$vpcStack$nameExtension5
    Subnet5=$(aws ec2 describe-subnets --filters "Name=tag:Name,Values=$subnetName5"  --query 'Subnets[*].SubnetId' --output text 2>&1)
    echo "SubnetId5: $Subnet5"
    ...
    dNS=$(aws route53 list-hosted-zones --query HostedZones[].{Name:Name} --output text)
    echo "dnsDomain = $dNS"

    CERTIFICATE=$(aws acm list-certificates --query 'CertificateSummaryList[0].CertificateArn' --output text)
     echo "Certificate = $CERTIFICATE"
    noSecCERTIFICATE=$(aws acm list-certificates --query 'CertificateSummaryList[1].CertificateArn' --output text)
     echo "nowafCertificate = $noSecCERTIFICATE"
    ...
    aws cloudformation create-stack --stack-name $stackname --template-body file://csye6225-cf-auto-scaling-application.json --parameters ParameterKey=amiId,ParameterValue=$amiId ParameterKey=myVpc,ParameterValue=$vpcId ParameterKey=subnetId1,ParameterValue=$Subnet1 ParameterKey=subnetId2,ParameterValue=$Subnet2 ParameterKey=subnetId3,ParameterValue=$Subnet3 ParameterKey=subnetId4,ParameterValue=$Subnet4 ParameterKey=subnetId5,ParameterValue=$Subnet5 ParameterKey=bucketName,ParameterValue=$bucket ParameterKey=attachmentBucket,ParameterValue=$zone ParameterKey=bucketKey,ParameterValue=$bucketKey ParameterKey=domainName,ParameterValue=$domainName ParameterKey=dnsDomain,ParameterValue=$dNS ParameterKey=CERTIFICATE,ParameterValue=$CERTIFICATE ParameterKey=noSecCERTIFICATE,ParameterValue=$noSecCERTIFICATE --capabilities CAPABILITY_NAMED_IAM
    if [ $? -eq 0 ]
    then
        while true;
        do
